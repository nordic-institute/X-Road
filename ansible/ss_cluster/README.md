...
The playbook has been tested in AWS EC2 using stock RHEL 7, Ubuntu 14.04 and Ubuntu 18.04 AMIs running default X-Road security server installation. Other environments might require modifications to the playbook.
...
All the servers in a cluster should have the same operating system (Ubuntu 14.04, Ubuntu 18.04 or RHEL 7). The setup also assumes that the servers are in the same subnet. If that is not the case, one needs to modify master's pg_hba.nconf so that it accepts replication configurations from the correct network(s).
