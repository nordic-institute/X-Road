...
REMOTE_JETTY_SHA1SUM="$(curl -ss http://download.eclipse.org/jetty/9.3.6.v20151106/dist/jetty-distribution-9.3.6.v20151106.tar.gz.sha1)  $HOME/jetty/jetty.tgz"
...
if [[ ${REMOTE_JETTY_SHA1SUM} != ${LOCAL_JETTY_SHA1SUM} ]] ; then
  ...
  wget -O ~/jetty/jetty.tgz "http://download.eclipse.org/jetty/9.3.6.v20151106/dist/jetty-distribution-9.3.6.v20151106.tar.gz"
else
