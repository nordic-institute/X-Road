...
module RubyCertHelper
  ...
  class << self
    def included(base)
      return unless base.method_defined?(:helper_method)

      # When included in controller, make these methods available to
      # view helpers.
      base.helper_method :cert_object, :cert_dump, :cert_hash
    end
  end
