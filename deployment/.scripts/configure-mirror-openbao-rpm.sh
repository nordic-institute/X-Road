...
EOF
  ...
  if command -v curl >/dev/null; then
    ...
    echo "ERROR: Failed to fetch OpenBao GPG key from $OPENBAO_GPG_URL. Cannot configure OpenBao RPM repo."
    return 1
  }
...
EOF
  ...
  # Exclude openbao packages from EPEL repositories to prevent conflicts
  for repo_id in $(dnf repolist --enabled -q 2>/dev/null | awk '{print $1}' | grep -i epel); do
