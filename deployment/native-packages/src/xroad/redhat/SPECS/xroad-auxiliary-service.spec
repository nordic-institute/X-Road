...
Name:               xroad-auxiliary-service
BuildArch:          noarch
Version:            %{xroad_version}
# release tag, e.g. 0.201508070816.el7 for snapshots and 1.el7 (for final releases)
Release:            %{rel}%{?snapshot}%{?dist}
Summary:            X-Road auxiliary service
...
%description
X-Road auxiliary service

%prep
rm -rf auxiliary-service
cp -a %{srcdir}/common/auxiliary-service .
...
mkdir -p %{buildroot}/usr/share/xroad/jlib/auxiliary-service
mkdir -p %{buildroot}/usr/share/xroad/scripts
mkdir -p %{buildroot}/etc/xroad
mkdir -p %{buildroot}/usr/share/xroad/bin
mkdir -p %{buildroot}/usr/share/doc/%{name}

cp -p %{_sourcedir}/auxiliary-service/xroad-*.service %{buildroot}%{_unitdir}
cp -p -r %{srcdir}/../../../../src/service/auxiliary-service/auxiliary-service-application/build/quarkus-app/* %{buildroot}/usr/share/xroad/jlib/auxiliary-service
...
ln -s /usr/share/xroad/jlib/auxiliary-service/quarkus-run.jar %{buildroot}/usr/share/xroad/jlib/auxiliary-service.jar
...
%config /etc/xroad/services/auxiliary-service.conf

%defattr(-,root,root,-)
%attr(644,root,root) %{_unitdir}/xroad-auxiliary-service.service

%attr(550,root,xroad) /usr/share/xroad/bin/xroad-auxiliary-service
...
%systemd_post xroad-auxiliary-service.service

%execute_init_or_update_resources

%preun
%systemd_preun xroad-auxiliary-service.service

%postun
%systemd_postun_with_restart xroad-auxiliary-service.service
