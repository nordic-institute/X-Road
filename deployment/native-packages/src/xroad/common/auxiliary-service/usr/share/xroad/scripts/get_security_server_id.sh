...
select id.xroad_instance, id.member_class, id.member_code, s.servercode
from "${db_schema}".serverconf s
join "${db_schema}".client c on s.owner=c.id
join "${db_schema}".identifier id on c.identifier=id.id
where id.xroad_instance IS NOT NULL AND id.member_class IS NOT NULL AND
id.member_code IS NOT NULL AND s.servercode IS NOT NULL;
EOF
else
  exit 1
fi
