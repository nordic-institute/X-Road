...
echo "=====> Run container"
docker run ${external_db_args} --detach -p $2:4000 -p $httpport:80 -p 5588:5588 --network xroad-network -e XROAD_TOKEN_PIN=$3 -e XROAD_ADMIN_USER=$4 -e XROAD_ADMIN_PASSWORD=$5 -e XROAD_DB_HOST=$postgresqlhost -e XROAD_DB_PORT=$postgresqlport -e XROAD_DB_PWD="Aflafl90!!"  -e XROAD_LOG_LEVEL="INFO" --name $1 niis/xroad-security-server-sidecar:6.23.0-slim
