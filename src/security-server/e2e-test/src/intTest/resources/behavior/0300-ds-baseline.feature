...
Feature: 0300 - Data spaces baseline
  ...
  Scenario: Issuer service is provisioned
    Given Issuer Service participant context "issuer" with DID "did:web:ds-issuer-service%3A10100:issuer" and issuer service endpoint "http://ds-issuer-service:10012/api/issuance/v1alpha/participants/issuer" is created on "aux"
  ...
  Scenario: SS0 Identity Hub is provisioned
    Given Identity Hub participant context "test-part-ctx" with DID "did:web:ss0-ds-identity-hub%3A10100" and credential service endpoint "http://ss0-ds-identity-hub:10001/api/credentials/v1/participants/test-part-ctx" is created on "ss0"
  ...
  Scenario: SS1 Identity Hub is provisioned
    Given Identity Hub participant context "test-part-ctx" with DID "did:web:ss1-ds-identity-hub%3A10100" and credential service endpoint "http://ss1-ds-identity-hub:10001/api/credentials/v1/participants/test-part-ctx" is created on "ss1"
  ...
  Scenario: Consumer retrieves data through data space
    Given Participant context "test-part-ctx" with DID "did:web:ss0-ds-identity-hub%3A10100" is created on "ss0"
    And Participant context "test-part-ctx" config with DID "did:web:ss0-ds-identity-hub%3A10100" is created on "ss0"
    Then Catalog can be retrieved using participant context "test-part-ctx" on "ss0" from "did:web:ss1-ds-identity-hub%3A10100" on "ss1"
    And Contract negotiation is initiated using participant context "test-part-ctx" on "ss0" with provider "did:web:ss1-ds-identity-hub%3A10100" on "ss1"
    And Contract negotiation state is "FINALIZED" using participant context "test-part-ctx" on "ss0"
    And Transfer process is started using participant context "test-part-ctx" on "ss0" with provider "did:web:ss1-ds-identity-hub%3A10100" on "ss1"
  ...
  Scenario: Consumer acquires EDR via xroad-edr-api endpoint
    Then EDR is acquired via xroad-edr-api for context "test-part-ctx" on "ss0" from "did:web:ss1-ds-identity-hub%3A10100" on "ss1" for asset "asset-1"
