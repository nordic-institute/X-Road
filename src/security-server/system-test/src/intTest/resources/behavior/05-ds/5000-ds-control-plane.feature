...
Feature: 5000 - DS control plane tests
  ...
  Scenario: Identity Hub is provisioned
    ...
    #And Identity Hub participant context "test-part-ctx" with DID "did:web:ds-identity-hub%3A7183" is initialized with existing private key in vault with alias "test-part-ctx-key" and public key "-----BEGIN PUBLIC KEY-----\nMCowBQYDK2VwAyEAabMf5XIoieY8g7YxnS4QnQvz59Yz+2/8buyztuXAMgA=\n-----END PUBLIC KEY-----"
    Given Identity Hub participant context "test-part-ctx" with DID "did:web:ds-identity-hub%3A7183" is initialized and keypair is generated with private key alias "test-part-ctx-key"
    When Credential request "xroad-membership-credential-request" for credential definition "xroad-membership-credential-definition" of type "MembershipCredential" from issuer "did:web:ds-issuer-service%3A6183:issuer" is submitted for participant "test-part-ctx"
