@SoftToken
@RSA
Feature: 0110 - Signer: SoftToken: Key operations (RSA)

  Background:
    Given tokens are listed

  Scenario: Keys are generated
    When new RSA key "key-1" generated for token "soft-token-000"
    And name "First key" is set for generated key
    When new RSA key "key-2" generated for token "soft-token-000"
    And name "Second key" is set for generated key
    When new RSA key "key-3" generated for token "soft-token-000"
