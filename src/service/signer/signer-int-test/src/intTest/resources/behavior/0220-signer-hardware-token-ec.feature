@HardwareToken
@EC
Feature: 0220 - Signer: HardwareToken: Key operations (EC)
  ...
  Scenario: Previous Keys are deleted
    ...
    Then key "First key" is deleted from token "xrd-softhsm-0"
    And key "Second key" is deleted from token "xrd-softhsm-0"
    And key "KeyX" is deleted from token "xrd-softhsm-0"
    And key "BadAuthKey from CA" is deleted from token "xrd-softhsm-0"
    And key "SignKey from CA" is deleted from token "xrd-softhsm-0"

  Scenario: Keys are generated
    When new EC key "key-1" generated for token "xrd-softhsm-0"
    And name "First key" is set for generated key
    When new EC key "key-2" generated for token "xrd-softhsm-0"
    And name "Second key" is set for generated key
    When new EC key "key-3" generated for token "xrd-softhsm-0"
  ...
  Scenario: Key is deleted
    Given new EC key "key-X" generated for token "xrd-softhsm-0"
  ...
  Scenario: A key with Sign certificate is created
    Given new EC key "key-100" generated for token "xrd-softhsm-0"
  ...
  Scenario: A key with Auth certificate is not created in hardware token
    Given new EC key "key-200" generated for token "xrd-softhsm-0"
  ...
  Scenario: Self signed certificate is generated
    ...
    And token "xrd-softhsm-0" key "First key" has 1 certificates on secondary node
  ...
  Scenario: Self Signed Certificate can be (re)imported
    ...
    And token "xrd-softhsm-0" key "Second key" has 0 certificates on secondary node
