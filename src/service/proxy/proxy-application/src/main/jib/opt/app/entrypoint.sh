...
log "Generating new SSL key and certificate for the proxy"
"$XROAD_SCRIPT_LOCATION/generate_certificate.sh" -c $XROAD_SCRIPT_LOCATION -n internal -f -S -p 2>&1 >/dev/null | sed 's/^/   /'
