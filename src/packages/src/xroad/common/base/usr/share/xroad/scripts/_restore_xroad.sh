...
extract_to_tmp_restore_dir () {
  ...
  # reset permissions of all files to fixed, "safe" values, except in /etc/xroad/ssl we not change group rw privileges
  find "$RESTORE_DIR" -path "$RESTORE_DIR"/etc/xroad/ssl -prune -o -exec chmod a-x,o=,u=rwX,g=rX {} +
  chmod -R a-x,o=,u=rwX "$RESTORE_DIR"/etc/xroad/ssl
}
