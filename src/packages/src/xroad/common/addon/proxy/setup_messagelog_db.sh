...
setup_database() {
  ...
  local db_conn_user=$(get_db_prop 'username' "$db_default_user")
  local db_user=${db_conn_user%%@*}
  local db_schema=$(get_db_prop 'currentSchema' 'public')
  db_schema=${db_schema%%,*}
  ...
  local db_admin_conn_user=$(get_prop ${root_properties} "$db_name.database.admin_user" "$db_user")
  local db_admin_user=${db_admin_conn_user%%@*}
  local db_admin_password=$(get_prop ${root_properties} "$db_name.database.admin_password" "$db_password")
...
EOF
  ...
  if [[ $db_schema != "public" ]]; then
    crudini --set ${db_properties} '' "$db_name.hibernate.connection.currentSchema" "${db_schema},public"
  fi
...
setup_database "messagelog" "messagelog" "$1"
