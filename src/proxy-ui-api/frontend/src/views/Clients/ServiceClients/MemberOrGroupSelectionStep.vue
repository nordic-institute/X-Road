  ...
  <div class="xrd-tab-max-width xrd-view-common">
    ...
    <v-radio-group>
      <table class="xrd-table service-clients-table">
        ...
        <tbody>
          <tr v-for="candidate in this.filteredCandidates()" v-bind:key="candidate.id">
            <td class="checkbox-column">
              <div class="checkbox-wrap">
                <v-radio v-on:change="selectCandidate(candidate)"
            ...
            <td>{{candidate.service_client_type === 'LOCALGROUP' ? candidate.local_group_code : candidate.name}}</td>
            <td>{{candidate.id}}</td>
          </tr>
        </tbody>
      </table>
  ...
  methods: {
    fetchData(): void {
      api
        ...
        .then( (response: any) => this.serviceClientCandidates = response.data as ServiceClient[])
    ...
    filteredCandidates(): ServiceClient[] {
      return this.serviceClientCandidates
        .filter( (candidate: ServiceClient): boolean => {
          ...
          const id = candidate.service_client_type === 'LOCALGROUP'
            ? candidate.id.toString() : candidate.id.toLowerCase();
