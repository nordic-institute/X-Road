...
if command -v docker &>/dev/null; then
    ...
    docker run --rm -v $XROAD/..:/workspace -v /etc/passwd:/etc/passwd:ro -v /etc/group:/etc/group:ro -u $(id -u):$(id -g) -e HOME=/workspace/src/packages xroad-rpm-redhat-7 /workspace/src/packages/build-rpm.sh 7
    docker run --rm -v $XROAD/..:/workspace -v /etc/passwd:/etc/passwd:ro -v /etc/group:/etc/group:ro -u $(id -u):$(id -g) -e HOME=/workspace/src/packages xroad-rpm-redhat-8 /workspace/src/packages/build-rpm.sh 8
else
