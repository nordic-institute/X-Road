...
Feature: Security Server API
  ...
  Scenario: Get list of security servers
    ...
    And new security server 'CS:TEST:member-1:SS-X' authentication certificate registered from 'SECURITY_SERVER'
  ...
  Scenario: Get security server clients
    ...
    And new security server 'CS:TEST:member-2:SS-2' authentication certificate registered from 'SECURITY_SERVER'
    And management request is approved
