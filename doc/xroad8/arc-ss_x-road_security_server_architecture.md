...
| 5.08.2024  | 1.18    | Add X-Road 8 architecture changes                                                                    | Ričardas Bučiūnas  |
...
ds-data-plane is responsible for mediating messages between service clients and service providers. 
The messages are transmitted over the public Internet and the proxy ensures that the communication is secured using digital signatures and encryption.

#### 3.9.2 Encapsulated data

ds-data-plane configuration is stored in filesystem and secret store.

#### 3.9.3 Persistent data

ds-data-plane persists data transfer audit logs in the database.

### 3.10 ds-control-plane

#### 3.10.1 Role and responsibilities

ds-control-plane is responsible for access right management and negotiation of security policies between service clients and service providers.

#### 3.10.2 Encapsulated data

ds-control-plane configuration is stored in filesystem and secret store.

#### 3.10.3 Persistent data

ds-control-plane persists access right management audit logs in the database.

### 3.11 ds-identity-hub

#### 3.11.1 Role and responsibilities

ds-identity-hub is responsible for managing identities in a form of verifiable credentials (VCs).

#### 3.11.2 Encapsulated data

ds-identity-hub configuration is stored in filesystem and secret store.

#### 3.11.3 Persistent data

ds-identity-hub persists Verifiable Credentials (VCs) in the secret store (vault).

### 3.12 ds-secret-storage

#### 3.12.1 Role and responsibilities

ds-secret-storage is responsible for managing secrets.

#### 3.12.2 Encapsulated data

ds-secret-storage configuration is stored in filesystem and secret store.

#### 3.12.3 Persistent data

ds-secret-storage encrypts and stores secrets in the secret store (vault).
