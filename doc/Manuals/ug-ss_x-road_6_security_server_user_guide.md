...
- [4 Security Server Clients](#4-security-server-clients)
  ...
  - [4.7 Disabling Client Subsystem Temporarily](#47-disabling-client-subsystem-temporarily)
    - [4.7.1 Disabling Client Substystem](#471-disabling-client-substystem)
...
![](img/ug-ss_disabled.svg) **Disabled** - The assotion between the client and the Security server is disabled temporarily (see [4.7](#47-disabling-client-subsystem-temporarily)). From this state the client can mobe to the following states:
...
![](img/ug-ss_enabling_in_progress.svg)  **Enabling in progress** - a request is made from Security Server to enable client subsystem temporarily, but it has not propagated yet through global configuration. Once the configuration is propagated, client returns to "Registered" state.
...
Security Server client subsystem in "Registered" state may be disabled temporarily for maintenance purposes. When subsystem is disabled it cannot use or provide X-Road services.
...
Client subsystem in "Disabled" state may be enabled again. When it is enabled in the Security Server it first enters "Enabling in progress" state. Once state change is propagated, "Registered" state is restored.

#### 4.7.1 Disabling Client Substystem
