...
class XROADChecker {
    ...
    private static final Logger LOG =
            LoggerFactory.getLogger(XROADChecker.class);
    ...
    private boolean internalCanActivate() throws Exception {
        ...
        if (!authCertExists) {
            logAndPrint("The security server does not have a registered"
                    + " authentication certificate. Please create an"
                    + " authentication certificate and register it at"
                    + " the X-Road central server.");
        }

        if (!ownerSignCertExists) {
            logAndPrint("Owner of the security server does not have signing"
                    + " certificate. Please create signing certificate for"
                    + " the owner.");
        }

        if (!tspExists) {
            logAndPrint("There are no configured time-stamping services."
                    + " Please configure at least one time-stamping service.");
        }
    ...
    private boolean checkValidGlobalConf() {
        if (!GlobalConf.isValid()) {
            logAndPrint("The security server does not have up-to-date global"
                    + " configuration. Please fix the configuration downloading"
                    + " before proceeding.");
            return false;
        }
        return true;
    ...
    private void startSignerClient() throws Exception {
        if (actorSystem == null) {
            ...
            Config config = ConfigFactory.load().getConfig("xroadchecker");

            LOG.debug("Akka using configuration: {}", config);
            actorSystem = ActorSystem.create("XROADChecker", config);
        }
