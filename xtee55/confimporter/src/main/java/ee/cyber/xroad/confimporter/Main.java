...
public final class Main {
    ...
    private static final String LOCK_FILE = "confimporter.lock";

    private static ConfImporter confImporter = new ConfImporter();
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            log.error("ConfImporter failed", e);
            System.err.println("ConfImporter failed: " + e.getMessage());
    ...
    private static ImporterAction aclOfService(String producer, String service) {
        return () -> {
            doInTransaction(session -> {
                try {
                    confImporter.importAcl(producer, service);
                    return null;
                } catch (Exception e) {
    ...
    private static ImporterAction[] aclOfProducer(String producer) {
        ...
        try {
            clients = confImporter.getServiceCodesToImport(producer);
        } catch (Exception e) {
    ...
    private static ImporterAction[] acl() throws IOException {
        List<String> producersNames = confImporter.getProducersNamesToImport();
    ...
    private static ImporterAction clients() {
        return () -> {
            doInTransaction(session -> {
                try {
                    confImporter.importClients();
                    return null;
                } catch (Exception e) {
                    throw translateException(e);
                }
    ...
    private static CommandLine parseArgs(String[] args) throws ParseException {
        ...
        mainOperations.addOption(new Option("clients",
                "import clients from 5.0 to 6.0"));
