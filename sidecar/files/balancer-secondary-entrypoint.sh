...
do
 sleep 5 || exit 1
 su -c "rsync -q -e 'ssh -i /etc/.ssh/id_rsa -o StrictHostKeyChecking=no -o ConnectTimeout=5 ' -avz --timeout=10 --delete-delay --exclude /etc/xroad/xroad.properties --exclude '/conf.d/node.ini' --exclude '*.tmp'  --delay-updates --log-file=/var/log/xroad/slave-sync.log xroad-slave@${XROAD_PRIMARY_DNS}:/etc/xroad/ /etc/xroad/" xroad
 RC=$?
done

#Create cron job for rsync every minute
echo "* * * * * xroad rsync -e 'ssh -i /etc/.ssh/id_rsa -o StrictHostKeyChecking=no -o ConnectTimeout=5' -avz --timeout=10 --delete-delay --exclude /etc/xroad/xroad.properties --exclude '/conf.d/node.ini' --exclude '*.tmp'  --delay-updates --log-file=/var/log/xroad/slave-sync.log xroad-slave@$XROAD_PRIMARY_DNS:/etc/xroad/ /etc/xroad/ 2>&1" > /etc/cron.d/xroad-state-sync
